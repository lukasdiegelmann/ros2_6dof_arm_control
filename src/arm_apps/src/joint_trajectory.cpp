...
namespace arm_apps {
  namespace {
    ...
    double computeSegmentDt(
        const std::vector<double>& q0,  // waypoint i-1
  ...
  PlannedTrajectory planJointTrajectory(const std::vector<double>& q_start,
                                        ...
                                        const JointLimits& limits, size_t num_waypoints,
                                        double vel_scale, double max_acc, double desired_duration) {
    /*
    ...
    for (size_t i = 0; i < traj.q.size(); ++i) {
      if (traj.q[i].size() != joint_names.size()) {
        throw std::runtime_error(
            "toRosTrajectoryMsg: waypoint size does not match joint_names size");
      }
    }

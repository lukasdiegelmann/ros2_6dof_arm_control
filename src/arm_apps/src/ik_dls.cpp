...
namespace arm_apps
{
    namespace
    {
        ...
        static Eigen::Matrix<double, 6, Eigen::Dynamic> numericJacobian(
            const UrdfChainFK &fk,
            const std::vector<double> &q,
            double eps)
        {
        ...
        const IkParams &params)
    {
        ...
        // target in Eigen
        const Eigen::Vector3d p_t(target_pose.position.x,
        ...
        for (int it = 0; it < params.max_iters; ++it)
        {
            ...
            Eigen::Matrix<double, 6, 6> A = JJt + (params.lambda * params.lambda) * Eigen::Matrix<double, 6, 6>::Identity();

...
namespace arm_apps {
  namespace {
    ...
    std::vector<Target> loadTargets(const std::string& yaml_path,
    ...
    geometry_msgs::msg::PoseStamped targetToPoseStamped(const Target& t,
  ...
  class DemoSequenceNode : public rclcpp::Node {
   public:
    ...
    DemoSequenceNode() : Node("demo_sequence_node") {
      ...
      this->declare_parameter<std::string>("controller_manager_service",
    ...
    bool waitForControllerActive() {
      ...
      auto client =
          this->create_client<controller_manager_msgs::srv::ListControllers>(service_name);

      RCLCPP_INFO(get_logger(), "Waiting for controller_manager service %s...",
                  service_name.c_str());
      ...
      RCLCPP_INFO(get_logger(), "Waiting for controller '%s' to become ACTIVE...",
      ...
      while (rclcpp::ok()) {
        ...
        const auto rc =
            rclcpp::spin_until_future_complete(shared_from_this(), fut, std::chrono::seconds(2));
        if (rc == rclcpp::FutureReturnCode::SUCCESS) {
          ...
          if (!found) {
            RCLCPP_WARN(get_logger(), "Controller '%s' not listed yet; retrying...",
                        controller_name.c_str());
          } else if (active) {
          ...
          RCLCPP_WARN(get_logger(), "Timeout/error while calling %s; retrying...",
                      service_name.c_str());
        }
    ...
    double sendTarget(const Target& tgt, size_t index) {
      ...
      RCLCPP_INFO(get_logger(),
                  "[%zu] sending target frame=%s pos=[%.3f %.3f %.3f] rpy=[%.3f %.3f %.3f] "
                  "vel_scale=%.3f max_acc=%.3f num_waypoints=%d desired_duration=%.3f",
                  index, tgt.frame_id.c_str(), tgt.position[0], tgt.position[1], tgt.position[2],
                  tgt.rpy[0], tgt.rpy[1], tgt.rpy[2], goal.vel_scale, goal.max_acc,
                  goal.num_waypoints, goal.desired_duration);

      rclcpp_action::Client<GoToPose>::SendGoalOptions opts;
      opts.feedback_callback = [this, index](
        ...
        RCLCPP_INFO(get_logger(), "[%zu] feedback stage=%s iters=%d pos_err=%.6f rot_err=%.6f",
                    index, feedback->stage.c_str(), feedback->iterations, feedback->pos_err,
                    feedback->rot_err);
      };
      ...
      RCLCPP_INFO(get_logger(),
                  "[%zu] result success=%s msg='%s' planned_duration=%.3f iters=%d pos_err=%.6f "
                  "rot_err=%.6f",
                  index, res.success ? "true" : "false", res.message.c_str(), res.planned_duration,
                  res.iterations, res.pos_err, res.rot_err);

      return res.success ? std::max(0.0, res.planned_duration) : 0.0;
    }

    double pauseSeconds() const { return this->get_parameter("pause_s").as_double(); }
...

  {
    ...
    while (rclcpp::ok()) {
      try {
        ...
        RCLCPP_WARN(node->get_logger(), "Failed to count subscribers (shutdown/race): %s",
                    e.what());
        break;
      }
  ...
  if (log_targets) {
    ...
    for (size_t i = 0; i < targets.size(); ++i) {
      ...
      RCLCPP_INFO(node->get_logger(), "  [%zu] frame=%s pos=[%.3f %.3f %.3f] rpy=[%.3f %.3f %.3f]",
                  i, t.frame_id.c_str(), t.position[0], t.position[1], t.position[2], t.rpy[0],
                  t.rpy[1], t.rpy[2]);
    }
  }
    ...
    if (home_index_param >= 0) {
      ...
      if (idx < targets.size()) {
        ...
        RCLCPP_WARN(node->get_logger(),
                    "home_index=%d out of range (targets=%zu); using last target as home",
                    home_index_param, targets.size());
      }
    }

    RCLCPP_WARN(node->get_logger(),
                "Attempting return-to-home after failure at index %zu (home_index=%zu)",
                failed_index, home_index);

...
//
// Example:
//   ros2 run arm_apps draw_circle_cartesian_node --ros-args -p frame_id:=base_link -p radius:=0.12
//   -p plane:=xy -p num_points:=80 -p loops:=2 -p vel_scale:=0.2 -p max_acc:=0.3 -p
//   point_duration:=0.3
...
namespace arm_apps {
  namespace {
    ...
    geometry_msgs::msg::PoseStamped makePose(const std::string& frame_id, const rclcpp::Time& stamp,
    ...
    std::optional<std::ofstream> openCsvIfEnabled(const rclcpp::Logger& logger, bool enabled,
      ...
      if (ec) {
        RCLCPP_WARN(logger, "Failed to create CSV directory '%s': %s",
  ...
  class DrawCircleCartesianNode : public rclcpp::Node {
   public:
    ...
    DrawCircleCartesianNode() : Node("draw_circle_cartesian_node") {
      ...
      joint_sub_ = this->create_subscription<sensor_msgs::msg::JointState>(
          this->get_parameter("joint_states_topic").as_string(), 10,
    ...
    void run() {
        ...
        while (rclcpp::ok()) {
          ...
          if (timeout_s > 0.0) {
            ...
            if (elapsed >= std::chrono::duration_cast<std::chrono::steady_clock::duration>(
              ...
              RCLCPP_WARN(get_logger(), "Timed out waiting for /joint_states after %.2fs",
                          timeout_s);
              break;
            }
          }
      ...
      RCLCPP_INFO(
          get_logger(),
          "Executing circle: plane=%s center=[%.3f %.3f %.3f] radius=%.3f points=%d loops=%d "
          "(total=%d)",
          plane_.c_str(), center_[0], center_[1], center_[2], radius_, num_points_, loops_,
          total_points);
   ...
   private:
    void loadAndValidateParams() {
      ...
      require(plane_ == "xy" || plane_ == "xz" || plane_ == "yz",
              "Parameter 'plane' must be one of: xy|xz|yz");
    ...
    bool sendOneGoal(int index, const GoToPose::Goal& goal) {
      ...
      options.feedback_callback = [this, index](
        ...
        RCLCPP_INFO_THROTTLE(get_logger(), *get_clock(), 2000,
                             "[%d] stage=%s iter=%d pos_err=%.4f rot_err=%.4f", index,
                             feedback->stage.c_str(), feedback->iterations, feedback->pos_err,
                             feedback->rot_err);
      };
      ...
      if (rc_handle != rclcpp::FutureReturnCode::SUCCESS) {
        RCLCPP_WARN(get_logger(), "[%d] Failed to send goal (future rc=%d)", index,
                    static_cast<int>(rc_handle));
      ...
      if (rc_result != rclcpp::FutureReturnCode::SUCCESS) {
        RCLCPP_WARN(get_logger(), "[%d] Failed waiting for result (future rc=%d)", index,
                    static_cast<int>(rc_result));
        writeCsv(index, goal.target, false, 0.0, 0, 0.0, 0.0);
        return false;
      }
      ...
      if (!result) {
        RCLCPP_WARN(get_logger(), "[%d] Empty result from server (code=%d)", index,
      ...
      if (success) {
        RCLCPP_INFO(get_logger(), "[%d] OK planned_duration=%.3f iter=%d pos_err=%.4f rot_err=%.4f",
                    index, result->planned_duration, result->iterations, result->pos_err,
                    result->rot_err);
      } else {
        RCLCPP_WARN(get_logger(),
                    "[%d] FAIL (code=%d) success=%s msg='%s' planned_duration=%.3f iter=%d "
                    "pos_err=%.4f rot_err=%.4f",
                    index, static_cast<int>(wrapped.code), result->success ? "true" : "false",
                    result->message.c_str(), result->planned_duration, result->iterations,
                    result->pos_err, result->rot_err);
      }

      writeCsv(index, goal.target, success, result->planned_duration, result->iterations,
               result->pos_err, result->rot_err);
      return success;
    }

    void writeCsv(int index, const geometry_msgs::msg::PoseStamped& pose, bool success,
                  double planned_duration, int iterations, double pos_err, double rot_err) {
      if (!csv_out_.has_value()) {
        return;
      }

      auto& out = csv_out_.value();
      out << index << ',' << pose.pose.position.x << ',' << pose.pose.position.y << ','
          << pose.pose.position.z << ',' << (success ? 1 : 0) << ',' << planned_duration << ','
          << iterations << ',' << pos_err << ',' << rot_err << '\n';
      out.flush();
    }

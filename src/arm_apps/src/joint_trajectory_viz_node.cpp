...
public:
  JointTrajectoryVizNode() : Node("joint_trajectory_viz_node") {
    ...
    this->declare_parameter<std::string>("commanded_topic",
    ...
    viz_pub_ = this->create_publisher<visualization_msgs::msg::MarkerArray>("/joint_traj_viz",
                                                                            rclcpp::QoS(10));
    ...
    if (show_commanded_) {
      traj_sub_ = this->create_subscription<trajectory_msgs::msg::JointTrajectory>(
          commanded_topic_, rclcpp::QoS(10),
    ...
    if (clear_service_enabled_) {
      clear_srv_ = this->create_service<std_srvs::srv::Empty>(
          ...
          [this](const std::shared_ptr<std_srvs::srv::Empty::Request>,
                 std::shared_ptr<std_srvs::srv::Empty::Response>) { clearAll(); });
    }

    const auto period = std::chrono::duration_cast<std::chrono::nanoseconds>(
        std::chrono::duration<double>(1.0 / publish_rate_hz_));
    publish_timer_ = this->create_wall_timer(period, [this]() { publishMarkers(); });
...
private:
  ...
  bool shouldPushSample(const std::string& name, const rclcpp::Time& t) {
    ...
    const bool stride_ok =
        (sample_stride_ <= 1) ? true : ((cnt % static_cast<uint64_t>(sample_stride_)) == 0);
    if (!stride_ok) {
      return false;
    }
  ...
  std_msgs::msg::ColorRGBA colorForIndex(std::size_t idx, float alpha) const {
    const float h = (joint_order_.empty())
                        ? 0.0f
                        : (static_cast<float>(idx) / static_cast<float>(joint_order_.size()));
  ...
  visualization_msgs::msg::Marker makeLineMarker(const std::string& ns, int id,
  ...
  visualization_msgs::msg::Marker makeTextMarker(const std::string& ns, int id,

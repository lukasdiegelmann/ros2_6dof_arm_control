...
namespace
{
                                ...
                                const arm_apps::IkResult &result)
    {
        ...
        out << "# success=" << (result.success ? "true" : "false")
            ...
            << ", final_pos_err_m=" << result.pos_err
...
int main(int argc, char **argv)
{
    ...
    try
    {
        ...
        const std::filesystem::path output_rot_csv = out_dir / (ts + "_ik_dls_rotation_convergence.csv");
        ...
        // Rotation-only CSV (iteration,rot_err_rad)
        {
            ...
            if (!out_rot)
            {
                throw std::runtime_error("Failed to open rotation CSV for writing: " + output_rot_csv.string());
            }
        ...
        RCLCPP_INFO(node->get_logger(), "IK rotation convergence CSV written to '%s'", output_rot_csv.string().c_str());

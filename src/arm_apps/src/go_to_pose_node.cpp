...
class GoToPoseNode : public rclcpp::Node {
 public:
  ...
  GoToPoseNode() : Node("go_to_pose_node"), joint_limits_(fk_) {
    ...
    this->declare_parameter<std::string>("trajectory_topic",
    ...
    RCLCPP_INFO(get_logger(), "FK ready. DOF=%zu, base=%s, tip=%s", fk_.dof(), base_link.c_str(),
                tip_link.c_str());
    ...
    joint_sub_ = this->create_subscription<sensor_msgs::msg::JointState>(
        "/joint_states", 10,
        [this](sensor_msgs::msg::JointState::SharedPtr msg) { onJointState(msg); });
    ...
    action_server_ = rclcpp_action::create_server<GoToPose>(
        ...
        [this](std::shared_ptr<GoalHandleGoToPose> goal_handle) { handle_accepted(goal_handle); });
 ...
 private:
  ...
  void publish_feedback(const std::shared_ptr<GoalHandleGoToPose>& goal_handle,
                        const std::string& stage, int iterations, double pos_err, double rot_err) {
    if (!rclcpp::ok()) {
      return;
    }
    ...
    try {
      ...
      if (goal_handle->is_canceling()) {
        ...
        try {
          ...
          RCLCPP_WARN(get_logger(), "Failed to publish canceled result (shutdown/race): %s",
                      e.what());
        }
        return;
      }
       ...
       */
      {
        ...
        if (!have_js_) {
          ...
          try {
            ...
            RCLCPP_WARN(get_logger(), "Failed to publish abort result (shutdown/race): %s",
                        e.what());
          }
          return;
        }
      ...
      const std::vector<double>& q_init =
          last_solution_copy.has_value() ? *last_solution_copy : q_start;
      ...
      if (ik.iterations > static_cast<int>(0.7 * static_cast<double>(p.max_iters))) {
        ...
        RCLCPP_WARN(get_logger(), "Hard IK (%d/%d). Increasing damping to %.4f", ik.iterations,
                    p.max_iters, lambda_);
      }
       ...
       */
      try {
        planned = arm_apps::planJointTrajectory(q_start, ik.q, joint_limits_, num_waypoints,
                                                vel_scale, max_acc, desired_duration);
      } catch (const std::exception& e) {
        result->success = false;
      ...
      auto traj_msg =
          arm_apps::toRosTrajectoryMsg(planned, fk_.jointNames(), rclcpp::Time(0, 0, RCL_ROS_TIME));
      ...
      if (perf_warn_ms > 0) {
        const auto convert_ms =
            std::chrono::duration_cast<std::chrono::milliseconds>(t_convert1 - t_convert0).count();
        const auto publish_ms =
            std::chrono::duration_cast<std::chrono::milliseconds>(t_pub1 - t_pub0).count();
        if (convert_ms >= perf_warn_ms || publish_ms >= perf_warn_ms) {
          RCLCPP_WARN(
              get_logger(),
              "Slow PUBLISHING step: toRosTrajectoryMsg=%ldms publish=%ldms (threshold=%dms). This "
              "can explain perceived lag between PUBLISHING and EXECUTING.",
              static_cast<long>(convert_ms), static_cast<long>(publish_ms), perf_warn_ms);
        }
      }
      ...
      if (this->get_parameter("wait_for_execution").as_bool() && planned_duration > 0.0) {
        ...
        const int period_ms_raw =
            static_cast<int>(this->get_parameter("execution_check_period_ms").as_int());
        ...
        const double stall_warn_s =
            std::max(0.0, this->get_parameter("execution_stall_warn_s").as_double());
        rclcpp::Time last_ros_time = start_time;

        const auto start_steady = std::chrono::steady_clock::now();
        const auto end_steady =
            start_steady + std::chrono::duration_cast<std::chrono::steady_clock::duration>(
        ...
        while (rclcpp::ok()) {
          ...
          if (stall_warn_s > 0.0) {
            ...
            if (q_now.has_value() && q_last.has_value() && q_now->size() == q_last->size()) {
              ...
              if (max_abs_dq > 1e-4) {
                ...
                if (motion_was_stalled) {
                  ...
                  if (std::isfinite(stall) && stall >= stall_warn_s) {
                    RCLCPP_WARN(get_logger(),
                                "Joint motion resumed after %.3fs wall (during EXECUTING)", stall);
                  }
                }
                q_last = q_now;
            ...
            if (std::isfinite(since_warn_s) && since_warn_s >= stall_warn_s) {
              const double clock_stall_wall_s =
                  toSeconds(steady_now - last_ros_time_progress_steady);
              const double joint_stall_wall_s = toSeconds(steady_now - last_joint_progress_steady);
              const double js_rx_age_wall_s =
                  have_js_recv ? toSeconds(steady_now - js_recv_steady) : -1.0;
              ...
              if (std::isfinite(clock_stall_wall_s) && clock_stall_wall_s >= stall_warn_s) {
                ...
                if (!using_wall_deadline && this->get_parameter("use_sim_time").as_bool()) {
                  ...
                  RCLCPP_WARN(
                      get_logger(),
                      "Detected /clock stall for %.3fs wall; switching EXECUTING wait termination "
                      "to wall-clock deadline to avoid waiting on frozen sim time.",
                      clock_stall_wall_s);
                }
              }
              ...
              if (std::isfinite(js_rx_age_wall_s) && js_rx_age_wall_s >= stall_warn_s) {
                js_was_stalled = true;
              } else if (js_was_stalled && have_js_recv && std::isfinite(js_rx_age_wall_s) &&
                         js_rx_age_wall_s < (stall_warn_s * 0.5)) {
                RCLCPP_WARN(get_logger(),
              ...
              if ((std::isfinite(clock_stall_wall_s) && clock_stall_wall_s >= stall_warn_s) ||
                ...
                RCLCPP_WARN(
                    get_logger(),
                    "Execution stall (wall>=%.2fs): /clock_stall_wall=%.3fs "
                    "joint_move_stall_wall=%.3fs joint_states_rx_age_wall=%.3fs "
                    "(ros_time_now=%.3f, joint_state_stamp_age_ros=%.3fs)",
                    stall_warn_s, std::isfinite(clock_stall_wall_s) ? clock_stall_wall_s : -1.0,
                    std::isfinite(joint_stall_wall_s) ? joint_stall_wall_s : -1.0,
                    std::isfinite(js_rx_age_wall_s) ? js_rx_age_wall_s : -1.0, t_now.seconds(),
